...
class PolygonNestingEngine:
    ...
    def initial_orderings(self) -> List[Tuple[Item, ...]]:
        ...
        unique: List[Tuple[Item, ...]] = []
        seen = set()
        for order in [raw, by_hard, by_area, by_wide, by_tall]:
    ...
    def beam_search_pack(self, ordered_items: Tuple[Item, ...]) -> SearchState:
    ...
    def guided_pack(self, ordered_items: Tuple[Item, ...], strategy: str = "all_sw") -> SearchState:
        ...
        while state.remaining:
            ...
            for idx, item in enumerate(state.remaining):
                ...
                if not candidates:
                    continue
                state = self.place(state, candidates[0], idx)
                placed_any = True
                break
            if not placed_any:
                break
    ...
    def gap_fill_pass(self, state: SearchState, strategy: str = "all_sw") -> SearchState:
        ...
        for item in list(remaining):
            ...
            if not candidates:
                continue
            current = self.place(current, candidates[0], idx)
        return current
    ...
    def nest(self) -> SearchState:
        ...
        strategies = [
            "dominant_sw_others_se",
            "all_sw",
            "all_se",
            "dominant_sw_others_ne",
        ]
        orderings = self.initial_orderings()

        # Fast multi-start greedy passes usually find the best industrial-strip layout quickly.
        for order in orderings:
            for strategy in strategies:
                guided = self.guided_pack(order, strategy=strategy)
                guided = self.gap_fill_pass(guided, strategy=strategy)
        ...
        for order in orderings[: min(2, len(orderings))]:
            beam_state = self.beam_search_pack(order)
            beam_state = self.gap_fill_pass(beam_state, strategy="all_sw")
...
def run_nesting(data: dict, width: float, height: float, plot: bool = True,
    ...
    if plot:
        plot_arrangement(
            state.placements,
            plate_poly,
            title="Advanced polygon nesting (true NFP + beam search + edge-aware multi-start + gap filling)",
        )
    return state
...
class NestingApp:
    def __init__(self, root):
        ...
        self.root = root
        self.root.title("2D Polygon Nesting - true NFP + backtracking + edge-aware search")
...
# ----------------------------
def main():
    parser = argparse.ArgumentParser(description="Advanced 2D polygon nesting with true NFP, beam-search backtracking, and edge-aware multi-start search")
